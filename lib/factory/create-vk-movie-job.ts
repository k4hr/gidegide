...
export async function createVkMovieJob(input: CreateVkMovieJobInput) {
  ...
  const intervalMinutes = Math.max(15, Math.min(180, Math.round(input.scheduleIntervalMinutes)));
  const timeZone = input.timeZone || "Europe/Moscow";
  const schedule: ScheduleConfig | null = input.scheduleMode === "WINDOW"
    ? { type: "WINDOW_INTERVAL", startHour, endHour, intervalMinutes, timeZone }
    : null;
