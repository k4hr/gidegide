...
export async function GET() {
  try {
    const donors = await listSuperUploadDonors({ donorKind: STORY_SHORTS_DONOR_KIND });
    const candidates = await buildTodayCandidates({ limit: 10, donorKind: STORY_SHORTS_DONOR_KIND });
    ...
    return NextResponse.json({
      ...
      summary: {
        ...
        test: candidates.filter((video) => video.viralChance >= 60 && video.viralChance < 80).length,
    ...
    return NextResponse.json(
      { error: error instanceof Error ? error.message : "Не получилось загрузить Story donors" },
      { status: 500 },
    );
  }
}
...
export async function POST(request: Request) {
  try {
    ...
    return NextResponse.json({
      ...
      candidates,
      message: `Story donor сохранен: ${result.donor.channelTitle}. Видео найдено: ${result.analysis.totalSeen}`,
    });
  } catch (error) {
    ...
    return NextResponse.json(
      { error: error instanceof Error ? error.message : "Не получилось добавить Story donor" },
      { status: 500 },
    );
  }
}
...
export async function DELETE(request: Request) {
  try {
    ...
    return NextResponse.json({
      ...
      candidates,
      message: "Story donor выключен. История source videos сохранена.",
    });
  } catch (error) {
    if (error instanceof z.ZodError) {
      return NextResponse.json(
    ...
    return NextResponse.json(
      { error: error instanceof Error ? error.message : "Не получилось выключить Story donor" },
      { status: 500 },
    );
  }
}
